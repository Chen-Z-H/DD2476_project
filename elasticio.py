...
def searchArticles(query):
    ...
    conn.request("GET", searcharticleurl + "q=" + query + "&_source=title,id,categories&size=" + (str)(NUM_ARTICLES))
    ...
    out = []
    for hit in hits:
        out.append((hit["_source"]["id"],hit["_source"]["title"],hit["_source"]["categories"]))
    return out
